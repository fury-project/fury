...
public class StringMapSerializationSuite {
  ...
  public Object serialize(MapState state) {
    ...
    results[0] = state.fury.serialize(state.object1);
    results[1] = state.fury.serialize(state.object2);
    results[2] = state.fury.serialize(state.object3);
    return results;
  }
  ...
  public Object deserialize(MapState state) {
    Object[] results = new Object[3];
    results[0] = state.fury.deserialize(state.bytes1);
    results[1] = state.fury.deserialize(state.bytes2);
    results[2] = state.fury.deserialize(state.bytes3);
    return results;
  }
